...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  /**
   * Gerencia a interação com o card, forçando a abertura no navegador padrão
   * para evitar que o link abra dentro da "casca" do PWA standalone.
   */
  const handleCardInteraction = (link: LinkItem) => {
    onLinkClick(link);
    if (!link.isDownload) {
      // Força a abertura em nova aba/janela externa com políticas de segurança estritas
      const newWindow = window.open(link.url, '_blank', 'noopener,noreferrer');
      // Garante que a referência do opener seja limpa para segurança e performance
    ...
    <div className={`p-4 md:p-10 max-w-7xl mx-auto animate-fade-in mb-40 ${isDark ? 'text-white' : 'text-gray-900'}`}>
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-16 items-center">
        <div className="flex gap-4 items-center">
          {isProduction && (
            <img 
              src="https://i.postimg.cc/gJzFByk5/file-0000000098b8720e9deb64f615033168.png" 
              alt="Logo" 
          ...
          <div className="relative z-[100]">
            ...
            {isMenuOpen && (
              <div className={`absolute top-16 left-0 w-64 rounded-[2rem] p-4 shadow-2xl border z-[110] animate-fade-in ${isDark ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-gray-100'}`}>
                <button onClick={() => { setActiveCategory('all'); setIsMenuOpen(false); }} className={`w-full text-left px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest ${activeCategory === 'all' ? 'bg-blue-600 text-white' : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-900'}`}>TODOS</button>
                {CATEGORIES.map(cat => (
      ...
      <div className="mb-10 px-2">
         <h2 className="text-3xl font-black tracking-tighter italic uppercase">
           {showFavoritesOnly ? 'Meus Favoritos' : (CATEGORIES.find(c => c.id === activeCategory)?.name || 'TODOS')}
         </h2>
      </div>
